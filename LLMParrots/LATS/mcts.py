...
def run_mcts(
    ...
    for i, item in enumerate(dataset):
        ...
        root = Node(state_id=state_id, state=initial_state) # initial solution (for pass@1 metric)
        logger.info("Root:")
        logger.info(str(root))
        #Create depth 1 children
        logger.info("Creating initial children nodes")
        next_action_prompt = get_next_action_prompt(root.state)
        ...
        for action in next_actions:
            ...
            child_node = Node(state_id = state_id, state=child_state, parent=root)
        ...
        for iter in range(max_iters):
            ...
            if node.visits > 0:
                ...
                next_action_prompt = get_next_action_prompt(node.state)
                ...
                for action in next_actions:
                    state_id += 1
                    next_state_prompt = get_next_state_prompt(initial_state, action)
                    child_state = getActions(next_state_prompt)
                    child_node = Node(state_id=state_id, state=child_state, parent=node)
                    child_node.set_trajectory(action)
        ...
        if solved:
            ...
            item['solution_plan'] = solution_plan
            logger.info(f"Interation exhausted, best solution : {solution_plan}")
